...
function MembresiasPanel({ usuarios, onRefresh }) {
  ...
  const cancelarAsignacion = async (id, nombre) => {
    if (!confirm(`¿Cancelar la membresía de ${nombre}?`)) return;
    try { await apiFetch(`/membresias/asignaciones/${id}`, { method: "DELETE" }); loadData(); } catch (e) {}
  };
  ...
  const estadoBadge = {
    ACTIVA: { color: "var(--verde)", bg: "rgba(39,174,96,0.1)", border: "rgba(39,174,96,0.3)" },
    VENCIDA: { color: "var(--gris)", bg: "rgba(100,100,100,0.1)", border: "rgba(100,100,100,0.2)" },
    CANCELADA: { color: "var(--rojo)", bg: "rgba(192,57,43,0.1)", border: "rgba(192,57,43,0.3)" },
    AGOTADA: { color: "var(--oro)", bg: "rgba(212,168,67,0.1)", border: "rgba(212,168,67,0.3)" },
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {[["planes", "Planes"], ["asignaciones", `Usuarios (${asignaciones.filter(a => a.estado === "ACTIVA").length} activos)`]].map(([t, label]) => (
          <button key={t} className={`filter-btn ${tab === t ? "active" : ""}`} onClick={() => setTab(t)}>{label}</button>
        ))}
      </div>
      ...
      {tab === "planes" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
          {membresias.map((m, i) => (
            <div key={m.id} style={{ background: "var(--negro2)", border: `1px solid ${i % 2 === 1 ? "var(--rojo)" : "rgba(255,255,255,0.08)"}`, padding: 28, position: "relative", display: "flex", flexDirection: "column" }}>
              {i % 2 === 1 && <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", background: "var(--rojo)", color: "var(--blanco)", fontSize: 9, letterSpacing: 3, textTransform: "uppercase", padding: "4px 16px", fontFamily: "'Barlow Condensed', sans-serif" }}>Más Popular</div>}
              <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 26, letterSpacing: 3, marginTop: i % 2 === 1 ? 12 : 0 }}>{m.nombre}</div>
              {m.descripcion && <div style={{ fontSize: 11, color: "var(--gris)", letterSpacing: 1, fontStyle: "italic", marginBottom: 8 }}>{m.descripcion}</div>}
              <div style={{ display: "flex", alignItems: "baseline", gap: 6, margin: "8px 0" }}>
                <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 36, color: "var(--rojo)", lineHeight: 1 }}>${Number(m.precio).toLocaleString("es-CO")}</span>
                <span style={{ fontSize: 11, color: "var(--gris)", letterSpacing: 2 }}>/ {m.cortesIncluidos} cortes</span>
              </div>
              {m.descuento > 0 && <div style={{ fontSize: 11, color: "var(--verde)", letterSpacing: 1, marginBottom: 4 }}>✓ {m.descuento}% descuento en servicios</div>}
              <div style={{ height: 1, background: "rgba(255,255,255,0.06)", margin: "8px 0" }} />
              {m.beneficios && (
                <div style={{ flex: 1, marginBottom: 16 }}>
                  {m.beneficios.split('\n').filter(b => b.trim()).map((b, j) => (
                    <div key={j} style={{ fontSize: 11, color: "rgba(240,236,228,0.7)", marginBottom: 6, display: "flex", gap: 8 }}>
                      <span style={{ color: "var(--rojo)" }}>✓</span> {b.trim()}
                    </div>
                  ))}
                </div>
              )}
              <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                <button className="btn-sm btn-edit" onClick={() => openEdit(m)} style={{ flex: 1 }}>Editar</button>
                <button className="btn-sm btn-delete" onClick={() => eliminar(m.id)} style={{ flex: 1 }}>Eliminar</button>
              </div>
            </div>
          ))}
      ...
      {/* ── ASIGNACIONES — cards con barra de progreso ── */}
      {tab === "asignaciones" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
          {asignaciones.map(a => {
            const pct = Math.min(100, Math.round((a.cortesUsados / (a.membresia?.cortesIncluidos || 1)) * 100));
            const diasRestantes = Math.max(0, Math.ceil((new Date(a.fechaFin) - new Date()) / (1000 * 60 * 60 * 24)));
            const badge = estadoBadge[a.estado] || estadoBadge.CANCELADA;
            const alerta = a.estado === "ACTIVA" && (diasRestantes <= 7 || (a.membresia?.cortesIncluidos - a.cortesUsados) <= 1);
            return (
              <div key={a.id} style={{ background: "var(--negro2)", border: `1px solid ${alerta ? "rgba(212,168,67,0.4)" : "rgba(255,255,255,0.06)"}`, padding: 20 }}>
                {alerta && (
                  <div style={{ fontSize: 10, color: "var(--oro)", letterSpacing: 2, marginBottom: 10, textTransform: "uppercase", fontFamily: "'Barlow Condensed', sans-serif" }}>
                    ⚠ {diasRestantes <= 7 ? `Vence en ${diasRestantes} días` : "Último corte disponible"}
                  </div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 16, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>{a.usuario?.nombre}</div>
                    <div style={{ fontSize: 11, color: "var(--gris)", marginTop: 2 }}>{a.usuario?.email}</div>
                    {a.usuario?.telefono && <div style={{ fontSize: 11, color: "var(--gris)" }}>📞 {a.usuario.telefono}</div>}
                  </div>
                  <span style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", padding: "4px 10px", color: badge.color, background: badge.bg, border: `1px solid ${badge.border}`, fontFamily: "'Barlow Condensed', sans-serif", whiteSpace: "nowrap" }}>{a.estado}</span>
                </div>

                <div style={{ background: "var(--negro3)", padding: "10px 14px", marginBottom: 12, border: "1px solid rgba(192,57,43,0.15)" }}>
                  <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 18, letterSpacing: 2, color: "var(--rojo)" }}>{a.membresia?.nombre}</div>
                  <div style={{ fontSize: 11, color: "var(--gris)", marginTop: 2 }}>{a.membresia?.cortesIncluidos} cortes · {a.membresia?.descuento}% descuento</div>
                </div>

                <div style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "var(--gris)", fontFamily: "'Barlow Condensed', sans-serif" }}>Cortes usados</span>
                    <span style={{ fontSize: 11, color: "var(--blanco)", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>{a.cortesUsados} / {a.membresia?.cortesIncluidos}</span>
                  </div>
                  <div style={{ height: 4, background: "var(--negro3)", borderRadius: 2 }}>
                    <div style={{ height: "100%", width: `${pct}%`, background: pct >= 80 ? "var(--rojo)" : "var(--verde)", borderRadius: 2, transition: "width 0.3s" }} />
                  </div>
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div>
                    <span style={{ fontSize: 10, color: "var(--gris)" }}>Vence: </span>
                    <span style={{ fontSize: 12, color: diasRestantes <= 7 ? "var(--oro)" : "var(--blanco)", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 600 }}>
                      {new Date(a.fechaFin).toLocaleDateString("es-CO", { day: "numeric", month: "short", year: "numeric" })}
                    </span>
                  </div>
                  <div>
                    <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: "var(--blanco)" }}>{Math.max(0, (a.membresia?.cortesIncluidos || 0) - a.cortesUsados)}</span>
                    <span style={{ fontSize: 10, color: "var(--gris)", fontFamily: "'Barlow Condensed', sans-serif", marginLeft: 4 }}>disponibles</span>
                  </div>
                </div>

                {a.estado === "ACTIVA" && (
                  <button className="btn-sm btn-cancel" onClick={() => cancelarAsignacion(a.id, a.usuario?.nombre)} style={{ width: "100%", padding: "8px" }}>
                    Cancelar Membresía
                  </button>
                )}
              </div>
          ...
          {asignaciones.length === 0 && <div className="empty" style={{ gridColumn: "1/-1" }}>No hay membresías asignadas aún.</div>}
        </div>
      )}
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <label className="field-label">Nombre</label>
            <input className="input" placeholder="Ej: VIP, BLACK PLUS" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))} />
            <label className="field-label">Precio (COP)</label>
            <input className="input" type="number" placeholder="80000" value={form.precio} onChange={e => setForm(p => ({ ...p, precio: e.target.value }))} />
            <label className="field-label">Cortes incluidos</label>
            ...
            <label className="field-label">Descripción corta</label>
            <input className="input" placeholder="Ej: Solo Corte · 1 Mes" value={form.descripcion} onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} />
            <label className="field-label">Beneficios (uno por línea)</label>
            <textarea className="input" rows={5} placeholder={"Corte incluido sin costo\n20% dto. productos\nReserva prioritaria"} value={form.beneficios} onChange={e => setForm(p => ({ ...p, beneficios: e.target.value }))} style={{ resize: "none" }} />
            {error && <div className="error-msg">{error}</div>}
            <div className="modal-actions">
              <button className="btn-full secondary" onClick={() => setShowModal(false)}>Cancelar</button>
              <button className="btn-full primary" onClick={save} disabled={loading}>{loading ? "Guardando..." : "Guardar"}</button>
      ...
      {showAsignarModal && (
        <div className="modal-overlay" onClick={() => setShowAsignarModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 12, color: "var(--gris)", marginBottom: 16, lineHeight: 1.6 }}>
              Asigna un plan al cliente después de confirmar el pago por WhatsApp. Máximo 2 meses de vigencia.
            </div>
            ...
            <select className="input" value={formAsignar.membresiaId} onChange={e => setFormAsignar(p => ({ ...p, membresiaId: e.target.value }))} style={{ cursor: "pointer" }}>
              ...
              {membresias.map(m => (
                <option key={m.id} value={m.id}>{m.nombre} — {m.cortesIncluidos} cortes · ${Number(m.precio).toLocaleString("es-CO")}</option>
              ))}
            </select>
            <label className="field-label">Fecha de vencimiento (máx. 2 meses)</label>
            <input className="input" type="date" value={formAsignar.fechaFin}
              onChange={e => setFormAsignar(p => ({ ...p, fechaFin: e.target.value }))}
